...
*/
let current_users: string[] = ['abdullah', 'ali', 'ahsan', 'zeeshan', 'eric'];
let new_users: string[] = ['Abdullah', 'Bairstow', 'Stuart', 'Jimmy', 'ben'];
for (let i = 0; i < new_users.length; i++) {
  let lower: string = new_users[i].toLowerCase();
  if (current_users.includes(lower)) {
    console.log(`The username ${new_users[i]} is not availabe, Please enter a new username.`);
  }
  else {
